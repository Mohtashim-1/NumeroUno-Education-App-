"""Setup ROSPA Practical Assessment print format and workspace shortcuts."""
...
PRINT_NAME = "ROSPA Practical Assessment Form"
WORKSPACE = "Forms"
HEADER = "ROSPA"
SHORTCUT_LABEL = "ROSPA Practical Assessment"
FORM_PAGE = "rospa-practical-assessment-form"


def _load_template():
	path = Path(__file__).parent / "doctype/rospa_practical_assessment/rospa_practical_assessment_template.json"
...
def _backfill_assessors():
	...
	if not frappe.db.has_column("ROSPA Practical Assessment", "assessor"):
		return

	rows = frappe.db.sql(
		"""
		select name, assessor, assessor_name, student_group, assessor_signature
		from `tabROSPA Practical Assessment`
		""",
		as_dict=True,
	)
	for row in rows:
		...
		if (
			...
			frappe.db.set_value("ROSPA Practical Assessment", row.name, values, update_modified=False)
