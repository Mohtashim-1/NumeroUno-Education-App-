frappe.provide("numerouno.off_road_practical");
...
frappe.pages["off-road-practical-assessment-form"].on_page_load = function (wrapper) {
	if (wrapper.off_road_practical_assessment_form) {
		return;
	}

	try {
		if (!$("#off-road-practical-assessment-css").length) {
			$(
				'<link id="off-road-practical-assessment-css" rel="stylesheet" type="text/css" href="/assets/numerouno/css/off_road_practical_assessment_form.css">'
			).appendTo("head");
		}
		...
		const page =
			wrapper.page && wrapper.page.main && wrapper.page.main.length
				...
				: frappe.ui.make_app_page({
						parent: wrapper,
						title: __("Off Road Practical Assessment"),
						single_column: true,
				  });

		page.main.addClass("off-road-practical-assessment-page");
		wrapper.off_road_practical_assessment_form = new numerouno.off_road_practical.Form(page);
	} catch (error) {
		$(wrapper)
			...
			.html(
				`<div class="p-4">
					<h4>${__("Off Road Practical Assessment")}</h4>
...
frappe.pages["off-road-practical-assessment-form"].on_page_show = function (wrapper) {
	if (!wrapper.off_road_practical_assessment_form) {
		frappe.pages["off-road-practical-assessment-form"].on_page_load(wrapper);
		return;
	}
	wrapper.off_road_practical_assessment_form.resolve_route_and_load();
};

numerouno.off_road_practical.Form = class {
	constructor(page) {
		this.page = page;
		this.doc = null;
		this.group = null;
	...
	back_to_group() {
		...
		frappe.router.replace_route("off-road-practical-assessment-form");
	}
	...
	show_group_view(default_group = null) {
		...
		this.page.set_title(__("Off Road Practical Assessment"));

		this.$root.html(`
			<div class="lvp-portal-header">
				<div>
					<div class="lvp-kicker">${__("Off Road and Desert")}</div>
	...
	load_group_students(prepare = false) {
		...
		const proceed = () => {
			frappe.call({
				method:
					"numerouno.numerouno.page.off_road_practical_assessment_form.off_road_practical_assessment_form_api.get_students",
				args: { student_group },
				freeze: true,
		...
		frappe.call({
			method: "numerouno.numerouno.page.off_road_practical_assessment_form.off_road_practical_assessment_form_api.prepare",
			args: { student_group },
			freeze: true,
	...
	fetch_form(args) {
		...
		frappe.call({
			method:
				"numerouno.numerouno.page.off_road_practical_assessment_form.off_road_practical_assessment_form_api.get_form_html",
			args,
			freeze: true,
			callback: (r) => {
				if (r.exc) {
				...
				if (this.doc.name) {
					frappe.router.replace_route("off-road-practical-assessment-form", this.doc.name);
				}
			},
			error: () => {
				this.loading_key = null;
	...
	save() {
		...
		frappe.call({
			method: "numerouno.numerouno.page.off_road_practical_assessment_form.off_road_practical_assessment_form_api.save_form",
	...
	submit_doc() {
		...
		frappe.confirm(__("Submit this assessment?"), () => {
			frappe.call({
				method: "numerouno.numerouno.page.off_road_practical_assessment_form.off_road_practical_assessment_form_api.submit",
