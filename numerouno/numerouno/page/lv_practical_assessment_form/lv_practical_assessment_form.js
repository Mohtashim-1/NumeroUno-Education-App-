frappe.provide("numerouno.lv_practical");
...
frappe.pages["lv-practical-assessment-form"].on_page_load = function (wrapper) {
	if (wrapper.lv_practical_assessment_form) {
		return;
	}

	try {
		if (!$("#lv-practical-assessment-css").length) {
			$(
				'<link id="lv-practical-assessment-css" rel="stylesheet" type="text/css" href="/assets/numerouno/css/lv_practical_assessment_form.css">'
			).appendTo("head");
		}
		...
		const page =
			wrapper.page && wrapper.page.main && wrapper.page.main.length
				...
				: frappe.ui.make_app_page({
						parent: wrapper,
						title: __("LV Practical Assessment"),
						single_column: true,
				  });

		page.main.addClass("lv-practical-assessment-page");
		wrapper.lv_practical_assessment_form = new numerouno.lv_practical.Form(page);
	} catch (error) {
		$(wrapper)
			...
			.html(
				`<div class="p-4">
					<h4>${__("LV Practical Assessment")}</h4>
...
frappe.pages["lv-practical-assessment-form"].on_page_show = function (wrapper) {
	if (!wrapper.lv_practical_assessment_form) {
		frappe.pages["lv-practical-assessment-form"].on_page_load(wrapper);
		return;
	}
	wrapper.lv_practical_assessment_form.resolve_route_and_load();
};

numerouno.lv_practical.Form = class {
	constructor(page) {
		this.page = page;
		this.doc = null;
		this.group = null;
	...
	back_to_group() {
		...
		frappe.router.replace_route("lv-practical-assessment-form");
	}
	...
	show_group_view(default_group = null) {
		...
		this.page.set_title(__("LV Practical Assessment"));

		this.$root.html(`
			<div class="lvp-portal-header">
				<div>
					<div class="lvp-kicker">${__("Light Vehicle")}</div>
	...
	load_group_students(prepare = false) {
		...
		const proceed = () => {
			frappe.call({
				method:
					"numerouno.numerouno.page.lv_practical_assessment_form.lv_practical_assessment_form_api.get_students",
				args: { student_group },
				freeze: true,
		...
		frappe.call({
			method: "numerouno.numerouno.page.lv_practical_assessment_form.lv_practical_assessment_form_api.prepare",
			args: { student_group },
			freeze: true,
	...
	fetch_form(args) {
		...
		frappe.call({
			method:
				"numerouno.numerouno.page.lv_practical_assessment_form.lv_practical_assessment_form_api.get_form_html",
			args,
			freeze: true,
			callback: (r) => {
				if (r.exc) {
				...
				this.page.set_title(`${__("LV Practical Assessment")} — ${title}`);
				if (this.doc.name) {
					frappe.router.replace_route("lv-practical-assessment-form", this.doc.name);
				}
			},
			error: () => {
				this.loading_key = null;
	...
	save() {
		...
		frappe.call({
			method: "numerouno.numerouno.page.lv_practical_assessment_form.lv_practical_assessment_form_api.save_form",
	...
	submit_doc() {
		...
		frappe.confirm(__("Submit this assessment?"), () => {
			frappe.call({
				method: "numerouno.numerouno.page.lv_practical_assessment_form.lv_practical_assessment_form_api.submit",
	...
	print_doc() {
		...
		frappe.set_route("print", "LV Practical Assessment", this.doc.name);
	}
};
