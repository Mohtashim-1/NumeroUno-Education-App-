...
class LVPracticalAssessment(Document):
	def validate(self):
...
def _load_template():
	path = Path(__file__).parent / "lv_practical_assessment_template.json"
...
def populate_from_student_group(docname, student_group):
	...
	if not docname or docname.startswith("new-"):
		frappe.throw("Please save the LV Practical Assessment first.")
