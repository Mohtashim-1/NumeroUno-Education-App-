frappe.ui.form.on("ROSPA Practical Assessment", {
	onload(frm) {
		if (frm.is_new() && !(frm.doc.criteria || []).length) {
			load_rospa_template(frm);
		}
	},

	refresh(frm) {
		frm.add_custom_button(__("Load Template"), () => load_rospa_template(frm), __("Actions"));
		if (frm.doc.student_group && !frm.is_new()) {
			frm.add_custom_button(__("Fill from Student Group"), () => {
				frappe.call({
					method:
						"numerouno.numerouno.doctype.rospa_practical_assessment.rospa_practical_assessment.populate_from_student_group",
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Open Simple Form"), () => {
				frappe.set_route("rospa-practical-assessment-form", frm.doc.name);
			}, __("Actions"));
		}
	},
...
function load_rospa_template(frm) {
	const unsaved = frm.is_new() || frm.doc.name?.startsWith("new-");
	frappe.call({
		method: "numerouno.numerouno.rospa_practical_assessment_setup.get_template_rows",
		freeze: true,
		callback(r) {
			if (r.exc) return;
