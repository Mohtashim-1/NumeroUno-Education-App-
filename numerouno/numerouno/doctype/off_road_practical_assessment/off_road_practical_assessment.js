frappe.ui.form.on("Off Road Practical Assessment", {
	onload(frm) {
	...
	refresh(frm) {
		...
		if (frm.doc.student_group && !frm.is_new()) {
			frm.add_custom_button(__("Fill from Student Group"), () => {
				frappe.call({
					method:
						"numerouno.numerouno.doctype.off_road_practical_assessment.off_road_practical_assessment.populate_from_student_group",
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Open Simple Form"), () => {
				frappe.set_route("off-road-practical-assessment-form", frm.doc.name);
			}, __("Actions"));
		}
	},
...
function load_lv_template(frm) {
	...
	frappe.call({
		method: "numerouno.numerouno.off_road_practical_assessment_setup.get_template_rows",
		freeze: true,
		callback(r) {
			if (r.exc) return;
