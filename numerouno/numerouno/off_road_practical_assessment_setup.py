"""Setup Off Road Practical Assessment print format and workspace shortcuts."""
...
PRINT_NAME = "Off Road Practical Assessment Form"
WORKSPACE = "Forms"
HEADER = "Off Road"
SHORTCUT_LABEL = "Off Road Practical Assessment"
FORM_PAGE = "off-road-practical-assessment-form"


def _load_template():
	path = Path(__file__).parent / "doctype/off_road_practical_assessment/off_road_practical_assessment_template.json"
...
def install():
	...
	for rel in (
		"doctype/off_road_practical_assessment_criterion/off_road_practical_assessment_criterion.json",
		"doctype/off_road_practical_assessment/off_road_practical_assessment.json",
		"page/off_road_practical_assessment_form/off_road_practical_assessment_form.json",
		"print_format/off_road_practical_assessment_form/off_road_practical_assessment_form.json",
	):
...
def _backfill_assessors():
	...
	if not frappe.db.has_column("Off Road Practical Assessment", "assessor"):
		return

	rows = frappe.db.sql(
		"""
		select name, assessor, assessor_name, student_group, assessor_signature
		from `tabOff Road Practical Assessment`
		""",
		as_dict=True,
	)
	for row in rows:
		...
		if (
			...
			frappe.db.set_value("Off Road Practical Assessment", row.name, values, update_modified=False)


def _ensure_print_format():
	html_path = Path(__file__).parent / "print_format/off_road_practical_assessment/off_road_practical_assessment.html"
	css_path = Path(__file__).parent / "print_format/off_road_practical_assessment/off_road_practical_assessment.css"
...
def _default_print_html():
	return """
<div class="lv-form">
  <div class="lv-title">{{ doc.form_title or "PRACTICAL ASSESSMENT – OFF ROAD AND DESERT" }}</div>
  <div class="lv-subtitle">{{ doc.form_subtitle or "ADNOC Off-road and Desert Assessment form" }}</div>
