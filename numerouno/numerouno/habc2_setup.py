"""Install HABC2 Examination Declaration print format and workspace shortcuts."""
...
def install():
	...
	for rel in (
		"doctype/habc2_learner/habc2_learner.json",
		"doctype/habc2_examination_declaration/habc2_examination_declaration.json",
		"page/habc2_examination_form/habc2_examination_form.json",
		"print_format/habc2_examination_declaration_form/habc2_examination_declaration_form.json",
	):
...
def after_migrate():
	if not Path(__file__).parent.joinpath("doctype/habc2_examination_declaration/habc2_examination_declaration.json").exists():
		return
	try:
		install()
	except Exception:
		frappe.log_error(title="HABC2 setup after_migrate")
...
def _ensure_print_format():
	html_path = Path(__file__).parent / "print_format/habc2_examination_declaration/habc2_examination_declaration.html"
	css_path = Path(__file__).parent / "print_format/habc2_examination_declaration/habc2_examination_declaration.css"
