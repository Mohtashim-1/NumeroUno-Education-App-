"""Setup LV Practical Assessment print format and workspace shortcuts."""
...
PRINT_NAME = "LV Practical Assessment Form"
WORKSPACE = "Forms"
HEADER = "Light Vehicle"
SHORTCUT_LABEL = "LV Practical Assessment"
FORM_PAGE = "lv-practical-assessment-form"
LEGACY_BULK_PAGE = "lv-practical-assessment-bulk"


def _load_template():
	path = Path(__file__).parent / "doctype/lv_practical_assessment/lv_practical_assessment_template.json"
...
def _ensure_workspace():
	...
	legacy_links = {LEGACY_BULK_PAGE}
	legacy_labels = {"LV Practical Bulk Create", "LV Practical Assesment Bulk"}
	workspace.shortcuts = [
		row
		for row in workspace.shortcuts
		if row.link_to not in legacy_links and (row.label or "") not in legacy_labels
	]
	...
	lv_labels = {SHORTCUT_LABEL, "LV Practical Bulk Create", "LV Practical Assesment Bulk"}
	content = [block for block in content if block.get("data", {}).get("shortcut_name") not in lv_labels]
	content = [block for block in content if HEADER not in (block.get("data", {}).get("text") or "")]
	content.append(
		{
  ...
  <table class="lv-footer">
    ...
    <tr>
      <td colspan="2">
        ...
        I consider that the above person {% if doc.achievement_status == "Has achieved" %}<b>has</b>{% else %}<b>has not</b>{% endif %} achieved a sufficient level of understanding of the Light Vehicle Professional Driving Theory course.<br>
